...
// The shipped documents are English; these three cases keep the Chinese labels
// on purpose, because they are the only coverage that a project bootstrapped
// from an earlier release still parses. Translating them drops that guarantee.
...
test('doctor preserves schema version 1 and strict warning semantics', (t) => {
  ...
  const conflicting = techStack.replace(
    'Decision mode: user-declared route',
    'Decision mode: ai-recommended route',
  );
  assert.notEqual(conflicting, techStack, 'the fixture route mode must be there to conflict with');
  fs.writeFileSync(techStackPath, conflicting);
