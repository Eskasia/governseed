# GovernSeed Project

> Governance foundations for agent-native projects.

This project was initialized with GovernSeed. The generator compatibility
identifier remains `agent-governance-starter`.

## Start Here

1. Read `START_HERE.md`.
2. Read canonical `AGENTS.md`, then the runtime adapter for the active agent.
3. Complete Q1-Q9, then follow `GATE-INTENT-001` and `GATE-ROUTE-001` from `AGENTS.md`.
4. Fill the required documents before implementation starts.
5. Run `doctor` from the starter repo after the documents are filled.

## Runtime

- Initialized agent: {{AGENT}}
- Init profile: {{PROFILE_NAME}}

## Required Documents

{{REQUIRED_DOCUMENTS}}

## Included Profile Documents

{{INCLUDED_PROFILE_DOCUMENTS}}

## Conditional Documents

{{CONDITIONAL_DOCUMENTS}}

## Validation

```bash
node governseed/scripts/doctor.mjs <this-project>
```

Treat warnings as unfinished governance work, not as approval to start implementation.
