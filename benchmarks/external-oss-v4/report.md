...
The merged dependency-cache workflow was dispatched exactly once from `main` at commit `08579d4c7fa662b93e9fbcd32cdc76659db82bac` as run `30704989330`. It failed in the `TASK-OSS-09` preparation job before cache creation: the sealed Paperless-ngx seed commit `5f5ab1beea4feaaecee7131b4b36185f09e2f53c` was not reachable from the public repository. The other two preparation jobs were cancelled by the matrix strategy; no cache receipt or offline verification was produced.
...
- Dependency cache preparation: BLOCKED / run `30704989330` failed during the Paperless-ngx seed fetch; V3 rehearsal cache manifests remain inherited only.
- Pilot lock: draft only; no placeholder or invented runtime identity was used.

## Why the gate is blocked

The required `networkNoneObserved`, `credentialEnvironmentClean`, `workspaceBoundaryObserved`, `dependencyCacheReadOnly`, `processCleanupObserved`, `cgroupEmpty`, `artifactPrivacyScanPass`, and `cleanupComplete` observations require a successful disposable Ubuntu workflow. The exact cache run failed before those checks and produced no cache artifacts. The failure is classified as a sealed task/seed identity defect, not as evidence of a cache or runner pass. Host identity, cgroup v2, resource capacity, and OCI digest were observed in the earlier preflight, but exact Codex version, binary hash, model ID, randomization seed, arm order, and measured cache identity remain unfrozen.
...
`REVISE_HARNESS` is not a V4 decision enum. V4 is a qualification gate, so the operational result is `BLOCKED: DEPENDENCY_CACHE_RUN_FAILED`; no treatment decision is made. The failed run is not automatically rerun. A corrected sealed seed identity must be reviewed as a new benchmark version and dispatched as a new run before any valid `benchmark-lock.json` can be created.
