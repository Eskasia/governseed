...
const EXPERIMENTAL_EVALUATOR_ENTRY = 'experimental/governance-impact/eval.mjs';
...
const ERROR_CONTRACT = Object.freeze({
  ...
  EXPERIMENTAL_ENTRY_REQUIRED: [2, 'The live paired evaluator is not part of the Core release unit.', `Run node ${EXPERIMENTAL_EVALUATOR_ENTRY} with the same arguments.`],
...
async function executeCommand(command, options, deps) {
  ...
  if (command === 'preflight' || command === 'run') fail('EXPERIMENTAL_ENTRY_REQUIRED');
...
// Engine seams consumed by the experimental live evaluator entry. Core never
// imports that entry; the dependency runs one way only.
export {
  ensureSafeDirectoryChain,
  fail,
  isPlainObject,
