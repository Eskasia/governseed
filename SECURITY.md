...
- Docker `NetworkMode=none` leaves only container loopback. A host process enters only the container network namespace through narrowly reviewed `sudo -n nsenter`, drops to the invoking host UID/GID before executing the relay, listens on loopback, and connects to the host-only Unix-domain socket from the host mount namespace. The UDS is not bind-mounted into the container.
