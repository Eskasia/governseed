...
import {
  ...
  PROVIDER_RESPONSE_TOKEN_CEILING,
  validateProviderResponse,
} from './provider-response-validation.mjs';
...
export const CREDENTIAL_PROXY_CANARY_INPUT = 'Return exactly the JSON object {"runtime_canary":"PASS"}.';
export const CREDENTIAL_PROXY_REQUEST_LIMIT = 1;
export const CREDENTIAL_PROXY_TIMEOUT_MS = 30_000;
export const CREDENTIAL_PROXY_TOKEN_CEILING = PROVIDER_RESPONSE_TOKEN_CEILING;

if (CREDENTIAL_PROXY_MODEL !== PROVIDER_RESPONSE_MODEL) {
  throw new Error('credential proxy and provider response validator model bindings diverge');
}
...
const PROVIDER_RESPONSE_REQUIRED_FIELDS = Object.freeze([
  'id',
  'object',
  'status',
  'model',
...
const NORMALIZED_RESPONSE_FIELDS = Object.freeze(['model', 'output_text', 'usage']);
const NORMALIZED_RESPONSE_USAGE_FIELDS = Object.freeze([
  'input_tokens',
  'output_tokens',
  'total_tokens',
]);
...
function durablePolicyDescriptor(policy) {
  return Object.freeze({
    ...
    response: Object.freeze({
      providerRequiredFields: Object.freeze([...PROVIDER_RESPONSE_REQUIRED_FIELDS]),
      providerAdditionalFieldsAllowed: true,
      normalizedAllowedFields: Object.freeze([...NORMALIZED_RESPONSE_FIELDS]),
      normalizedUsageFields: Object.freeze([...NORMALIZED_RESPONSE_USAGE_FIELDS]),
...
function validateBody(bytes, policy) {
  ...
  if (body.input !== CREDENTIAL_PROXY_CANARY_INPUT) fail('PROXY_BODY_MISMATCH');
...
export function createHostCredentialProxy(options) {
  ...
  const callUpstream = async (body, signal) => {
    ...
    try {
      ...
      if (error?.code === 'PROXY_RESPONSE_INVALID') fail('PROXY_RESPONSE_INVALID');
      if (error?.code === 'PROXY_UPSTREAM_FAILED') fail('PROXY_UPSTREAM_FAILED');
      throw error;
    }
    const normalizedBody = Buffer.from(JSON.stringify(normalizedResponse));
    return {
      statusCode: upstreamResponse.statusCode,
      contentType: 'application/json',
      body: normalizedBody,
      rawBody: responseBody,
      responseShape: normalizedResponse,
    ...
    try {
      ...
      try {
        ...
        recordReceipt({
          ...
          responseSha256: hashBytes(upstream.rawBody),
          requestBytes: rawRequestBody.length,
          responseBytes: upstream.rawBody.length,
          modelId: upstream.responseShape.model,
          statusCode: upstream.statusCode,
          latencyMs: Math.max(0, dependencies.clock.now() - requestStartedAt),
          tokenCounts: {
            input: upstream.responseShape.usage.input_tokens,
            output: upstream.responseShape.usage.output_tokens,
            total: upstream.responseShape.usage.total_tokens,
          },
